		...
		It("creates Runners when StaticHost state is Ready", func() {
			...
			for i := range instances {
				...
				Eventually(func(g Gomega) {
					...
					g.Expect(r.Labels).To(And(
						HaveKeyWithValue(constants.HostLabel, staticHostName),
						HaveKeyWithValue(constants.RunnerIdLabel, runnerID),
						HaveKeyWithValue(constants.RunnerTypeLabel, constants.RunnerTypeLabelStatic),
					))
