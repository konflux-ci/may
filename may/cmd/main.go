...
import (
	...
	"github.com/konflux-ci/may/internal/controller/binder"
	"github.com/konflux-ci/may/internal/controller/claimer"
...
func main() {
	...
	if namespace == "" {
		setupLog.Error(fmt.Errorf("WATCH_NAMESPACE is not set"), "missing required environment variable")
	}
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "b3b3690d.may.konflux-ci.dev",
	...
	sched := scheduler.New(mgr.GetClient(), mgr.GetScheme(), namespace)
	if err := schedulerctrl.SetupWithManager(mgr, setupLog, namespace, sched); err != nil {
		setupLog.Error(err, "unable to setup scheduler")
		os.Exit(1)
	}
	if err := provisioner.SetupWithManager(mgr, setupLog, namespace); err != nil {
		setupLog.Error(err, "unable to setup provisioner")
		os.Exit(1)
	}
	if err := binder.SetupWithManager(mgr, setupLog); err != nil {
		setupLog.Error(err, "unable to setup binder")
		os.Exit(1)
	}
	if err := (&claimer.ClaimerController{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to setup claimer")
		os.Exit(1)
	}
	if err := (&gater.GaterController{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to setup gater")
		os.Exit(1)
	}
	// nolint:goconst
