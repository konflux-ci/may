	...
	Context("When reconciling a resource", func() {
		...
		When("Host is draining", func() {
			...
			When("runners are claimed", func() {
				...
				It("should reflect the status of the claimed runners", func(ctx context.Context) {
					By("Asserting the status")
					Expect(k8sClient.Get(ctx, client.ObjectKeyFromObject(host), host)).To(Succeed())
					Expect(host.Status.Runners.Stopped).To(Equal(0)) // no stopped runners
					Expect(host.Status.Runners.Ready).To(Equal(0))   // no ready runners
				})
			...
			It("should collect errors on runner deletion failure", func(ctx context.Context) {
				...
				Expect(controllerReconciler.Reconcile(ctx, reconcile.Request{
					...
					MatchError(ContainSubstring(errorString)),
					MatchError(kerrors.IsForbidden, "forbidden"),
				))
			})
