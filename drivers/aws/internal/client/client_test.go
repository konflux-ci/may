...
var _ = Describe("validateAWSConfiguration", func() {
	When("region, secret, and system namespace are set", func() {
	...
	When("the secret and system namespace annotations are missing", func() {
		It("should return an error referencing both annotations", func() {
			err := validateAWSConfiguration(internalconfig.AWSConfiguration{
				Region: "us-east-1",
			})
			...
			Expect(err.Error()).Should(ContainSubstring(internalconfig.AnnotationSystemNamespace))
		})
	})
})
...
var _ = Describe("newEC2Client", func() {
	...
	When("required annotations are missing", func() {
	...
	return &maykonfluxcidevv1alpha1.StaticHost{
		ObjectMeta: metav1.ObjectMeta{
			...
			Annotations: map[string]string{
				...
				internalconfig.AnnotationSystemNamespace: "may-system",
			},
		},
	}
}
